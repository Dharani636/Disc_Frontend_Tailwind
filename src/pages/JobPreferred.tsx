...
export default function JobPreferred() {
  ...
  const loadData = async () => {
    const response = await api.get("/job-preferred");
  ...
  const save = async () => {
    if (!jobName) return;
    await api.post("/job-preferred", { jobName });
    setJobName("");
    loadData();
  };
  ...
  const saveEdit = async (id: number) => {
    ...
    await api.put(`/job-preferred/${id}`, { jobName: editingName });
    cancelEdit();
    loadData();
  };
  ...
  const remove = async (id: number) => {
    ...
    await api.delete(`/job-preferred/${id}`);
    loadData();
  };
